...
first\footcite{arxiv-2019,cbic-2021} was to conduct and publish rigorous (open
source, reproducible) benchmarks with popular datasets and state-of-the-art FCN
models in which we: i) showed that target domain fine-tuning is an effective
way to improve segmentation quality in unseen conditions; ii) current
state-of-the-art is bound by dataset sizes and very little is related to model
capacity.  We argue the performance of many contributions available in
literature is actually quite comparable within standard deviation margins of
each other, in spite of huge differences in the number of parameters for
different architectures.  Finally, we made our findings reproducible,
distributing code and documentation for future researchers to build upon, in
the hopes to inspire future work in the
